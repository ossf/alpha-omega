...
Last month, Tobias [announced](https://blog.rust-lang.org/2023/10/27/crates-io-non-canonical-downloads.html) that support for non-canonical downloads on crates.io would be removed. A non-canonical download is the ability to download a crate if you, for example, replace a hyphen in a name with an underscore - crates.io would just normalize those to be the same. Having non-canonical downloads prevented crates.io from using CDNs directly for download requests because of required database lookups that were required for this feature. Now that this is [disabled](https://github.com/rust-lang/crates.io/pull/7549), you have increased performance but, just as important, reliability, since the exact named crate being asked for is always the crate that is attempted to be downloaded.
